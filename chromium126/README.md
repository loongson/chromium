...
* `cross-toolchain (Release+Asserts-126.tar.bz2): `  点击[下载](http://ftp.loongnix.cn/browser/build/toolchain/Release+Asserts-126.tar.bz2)
...
`Release+Asserts-126.tar.bz2`替换`third_party/llvm-build/Release+Asserts`目录：

```shell
$ rm -rf third_party/llvm-build/Release+Asserts
$ tar -xjvf Release+Asserts-126.tar.bz2 -C third_party/llvm-build/
```

## 打入适配patch

* `适配patch (0001-la64-cross-CH126-Add-loongarch-build-support-for-old-new-w.patch): `  点击[下载](./0001-la64-cross-CH126-Add-loongarch-build-support-for-old-new-w.patch)

`0001-la64-cross-CH126-Add-loongarch-build-support-for-old-new-w.patch`文件打入源码：
...
上述patch基于126.0.6478.59构建生成，已包含ffmpeg配置文件的生成，patch打入成功则忽略ffmpeg配置。
...
至此，Chromium126构建配置已完成，您可以继续完成后面的[交叉构建](../#三构建配置)
