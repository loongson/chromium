...
* `rust-toolchain (rust-1.87.0-nightly-cross.tar.bz2): `  点击[下载](https://ftp.loongnix.cn/browser/build/toolchain/rust-1.87.0-nightly-cross.tar.bz2)
...
`rust-1.87.0-nightly-cross.tar.bz2`解压放入`/opt/`目录下：

```shell
$ tar -xjvf rust-1.87.0-nightly-cross.tar.bz2 -C /opt/
```
...
* `llvm (Release+Asserts-138.tar.bz2 社区配套CH144的编译器有bug，用CH138编译器正常构建): `  点击[下载](http://ftp.loongnix.cn/browser/build/toolchain/Release+Asserts-138.tar.bz2)
...
`Release+Asserts-138.tar.bz2`替换`third_party/llvm-build/Release+Asserts`目录：

```shell
$ rm -rf third_party/llvm-build/Release+Asserts
$ tar -xjvf Release+Asserts-138.tar.bz2 -C third_party/llvm-build/
```

## 打入适配patch

* `适配patch (0001-la64-cross-CH144-Add-loongarch-build-support-for-old-new-w.patch): `  点击[下载](./0001-la64-cross-CH144-Add-loongarch-build-support-for-old-new-w.patch)

`0001-la64-cross-CH144-Add-loongarch-build-support-for-old-new-w.patch`文件打入源码：

```shell
$ patch -Np1 -i 0001-la64-cross-CH144-Add-loongarch-build-support-for-old-new-w.patch
```
...
上述patch基于144.0.7559.251构建生成，已包含ffmpeg配置文件的生成，patch打入成功则忽略ffmpeg配置。
...
至此，Chromium144构建配置已完成，您可以继续完成后面的[交叉构建](../#三构建配置)
