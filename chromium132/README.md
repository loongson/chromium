...
* `rust-toolchain (rust-1.80.0-cross.tar.bz2): `  点击[下载](https://ftp.loongnix.cn/browser/build/toolchain/rust-1.80.0-cross.tar.bz2)
...
`rust-1.80.0-cross.tar.bz2`解压放入`/opt/`目录下：

```shell
$ tar -xjvf rust-1.80.0-cross.tar.bz2 -C /opt/
```

**注意：** 默认将交叉编译器工具链放入/opt/目录，可以任意指定目录，但若修改需要作如下相应调整：

> * llvm: 需要调整build/config/clang/clang.gni文件中`default_clang_base_path`变量
> * rust-toolchain: 需要调整build/cross-build.sh脚本中`rust_sysroot_absolute`参数
...
* `llvm (Release+Asserts-132.tar.bz2): `  点击[下载](http://ftp.loongnix.cn/browser/build/toolchain/Release+Asserts-132.tar.bz2)
* `rust-toolchain (rust-toolchain.tar.bz2): `  点击[下载](https://ftp.loongnix.cn/browser/build/toolchain/rust-toolchain.tar.bz2)

**注意：** 社区CH132自带的llvm和rust-toolchain有问题，需要全部替换。
...
`Release+Asserts-132.tar.bz2`替换`third_party/llvm-build/Release+Asserts`目录：

```shell
$ rm -rf third_party/llvm-build/Release+Asserts
$ tar -xjvf Release+Asserts-132.tar.bz2 -C third_party/llvm-build/
```

`rust-toolchain.tar.bz2`解压放入`/opt`目录下：

```shell
$ tar -xjvf rust-toolchain.tar.bz2 -C /opt/
```

**注意：** 默认将rust-toolchain工具链放入/opt/目录，可以任意指定目录，但若修改需要作如下相应调整：

> 需要调整build/cross-build.sh脚本中`rust_bindgen_root`和`rust_sysroot_absolute`参数


## 打入适配patch

* `适配patch (0001-la64-cross-CH132-Add-loongarch-build-support-for-old-new-w.patch): `  点击[下载](./0001-la64-cross-CH132-Add-loongarch-build-support-for-old-new-w.patch)

`0001-la64-cross-CH132-Add-loongarch-build-support-for-old-new-w.patch`文件打入源码：

```shell
$ patch -Np1 -i 0001-la64-cross-CH132-Add-loongarch-build-support-for-old-new-w.patch
```
...
上述patch基于132.0.6834.96构建生成，已包含ffmpeg配置文件的生成，patch打入成功则忽略ffmpeg配置。
...
**注意：** 完成以上配置后，需要在构建终端下设置如下变量：
```shell
$ export RUSTC_BOOTSTRAP=1
```


至此，Chromium132构建配置已完成，您可以继续完成后面的[交叉构建](../#三构建配置)
